...
% det(Numero, Genero, [determinante],lista).
...
%  sintagmas nominales.
%  nom(Numero, Genero,[nombre],lista).
...
%pron(Numero, Persona,Genero [pronombre],lista).
...
%verbo(Numero, Persona,[verbo],lista).
...
% ser verdaderas.
% R : Listas con la lecturas de frases de usuarios
% G : Lista con la la frase revisada por la gramatica
% O : Origen donde se encuentra el usuario
% D : Destino
% L : Lista con los puntos intermedios de la ruta
run :- tab(30),write("* * *"),nl,
    write("---  BIENVENIDO A WazeLog LA MEJOR FORMA DE LLEGAR A SU DESTINO"),
    nl,nl,write("- Por Favor indíqueme dónde se encuentra."),nl,
    tab(5),write(">"),
    leerLinea(R0),aSintac(R0,G0),analizarRes(G0,O), % Lectura para la primera pegunta
                                                    % La respuesta es correcta si lleva un nombre de lugar

    nl,write("- Muy bien, ¿Cuál es su destino?"),nl,
    tab(5),write(">"),
    leerLinea(R1),aSintac(R1,G1),analizarRes(G1,D), % Lectura de la segunda pregunta.
                                                    % La respuesta es correcta si lleva un nombre de algun lugar

    nl,write("- Excelente, ¿Tiene algún destino intermedio?"),
    nl,tab(5),write(">"),
    leerLinea(R2),aSintac(R2,G2),analizarResSec(G2,L),nl,   % Si/No o algun tipo de lugar

    % Preperacion de la ruta y respuesta que de da.
    inversa([D|L],L0),buscarRuta(O,L0,Rf,Dis),
    write("- Su ruta sería "),imprimirLista(Rf),write("la distancia es de "),
    write(Dis),write("km"),nl,
    write("- Gracias por usar WazeLog"),
    nl,nl,write("- Desea buscar otra ruta en WazeLog"),nl,
    tab(5),write(">"),
    leerLinea(R3),aSintac(R3,G3),write(G3),mainWazeLog(G3).

% ----------------------------------------------------------

%Regla qe escribe elemetos de una lista en pantalla.
imprimirLista([R|Resto]):- write(R),write(","),tab(1),
    imprimirLista(Resto).
